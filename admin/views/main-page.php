...
<div class="wrap rbco-wrap">
		...
		<div class="notice notice-error">
			<p>
				...
				printf(
					...
					esc_html__( 'Please %1$sconfigure your API key in Settings%2$s before creating content.', 'raybogman-content-orchestrator' ),
	...
	<div class="rbco-status-bar">
		<span class="rbco-status-item">
			<strong><?php esc_html_e( 'Provider:', 'raybogman-content-orchestrator' ); ?></strong>
			<?php echo esc_html( 'openai' === $rbco_provider ? 'OpenAI' : 'Claude' ); ?>
			(<?php echo esc_html( $rbco_model ); ?>)
		</span>
		<span class="rbco-status-item">
			<strong><?php esc_html_e( 'Yoast SEO:', 'raybogman-content-orchestrator' ); ?></strong>
			<?php if ( $rbco_has_yoast ) : ?>
				<span class="rbco-badge rbco-badge-success"><?php esc_html_e( 'Active', 'raybogman-content-orchestrator' ); ?></span>
			<?php else : ?>
				<span class="rbco-badge rbco-badge-warning"><?php esc_html_e( 'Not installed', 'raybogman-content-orchestrator' ); ?></span>
			<?php endif; ?>
		</span>
		<a href="<?php echo esc_url( admin_url( 'admin.php?page=rbco-settings' ) ); ?>" class="rbco-status-item rbco-status-link">
			<?php esc_html_e( 'Change Settings', 'raybogman-content-orchestrator' ); ?> &rarr;
		</a>
	</div>
	...
	<?php if ( ! empty( $rbco_project_vision ) ) : ?>
		<div class="notice notice-info" style="margin: 16px 0;">
			<p>
				...
				<strong><?php esc_html_e( 'Project Vision active', 'raybogman-content-orchestrator' ); ?></strong> &mdash;
				<?php esc_html_e( 'Your custom writing instructions are active and will be applied to all generated content.', 'raybogman-content-orchestrator' ); ?>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=rbco-settings' ) ); ?>"><?php esc_html_e( 'Edit', 'raybogman-content-orchestrator' ); ?></a>
			</p>
		...
		<div class="rbco-card">
			<div class="rbco-card-header">
				<h2>
					...
					<?php esc_html_e( 'Step 1: Website Scanning (optional)', 'raybogman-content-orchestrator' ); ?>
				</h2>
			</div>
			<div class="rbco-card-body">
				<p class="description" style="margin-bottom: 12px;">
					<?php esc_html_e( 'Enter a website URL to scan for context. The scanner will crawl sitemaps and internal links to extract content, headings, and metadata. This scanned data is used as context for the AI to generate relevant, on-brand content.', 'raybogman-content-orchestrator' ); ?>
				</p>
				<table class="form-table" role="presentation">
					<tbody>
						<tr>
							<th scope="row">
								<label for="rbco-url"><?php esc_html_e( 'Website URL', 'raybogman-content-orchestrator' ); ?></label>
							</th>
							<td>
								<input type="<?php echo rbco_is_pro() ? 'text' : 'url'; ?>" id="rbco-url" class="large-text" placeholder="https://example.com" />
								<p class="description">
									<?php if ( rbco_is_pro() ) : ?>
										<?php esc_html_e( 'Separate multiple URLs with commas. Leave empty to skip scanning.', 'raybogman-content-orchestrator' ); ?>
									<?php else : ?>
										<?php esc_html_e( 'Enter one website URL to scan for context. Leave empty to skip scanning.', 'raybogman-content-orchestrator' ); ?>
										<span style="background:#E4405F;color:#fff;padding:1px 6px;border-radius:8px;font-size:10px;font-weight:600;">ENT</span> <?php esc_html_e( 'Multiple URLs', 'raybogman-content-orchestrator' ); ?>
									<?php endif; ?>
								</p>
						...
						<tr>
							<th scope="row">
								<?php esc_html_e( 'PDF Sources', 'raybogman-content-orchestrator' ); ?>
							</th>
							<td>
								<div class="rbco-pdf-upload-area">
									...
									<button type="button" id="rbco-pdf-upload-btn" class="button">
										...
										<?php esc_html_e( 'Upload PDF', 'raybogman-content-orchestrator' ); ?>
									</button>
								...
								<p class="description" style="margin-top: 6px;">
									...
									printf(
										...
										esc_html__( 'Upload a PDF to use as context for content generation. Max file size: %s. Text will be extracted automatically.', 'raybogman-content-orchestrator' ),
								...
								<div id="rbco-pdf-library" class="rbco-pdf-library" style="margin-top: 12px;">
									<p class="description" style="margin-bottom: 8px;">
										<strong><?php esc_html_e( 'Saved PDFs — check to use as source:', 'raybogman-content-orchestrator' ); ?></strong>
									</p>
									<?php foreach ( $rbco_pdf_library as $rbco_pdf ) : ?>
										<div class="rbco-pdf-item" data-pdf-id="<?php echo esc_attr( $rbco_pdf['id'] ); ?>">
											...
											<button type="button" class="rbco-pdf-delete-btn" data-pdf-id="<?php echo esc_attr( $rbco_pdf['id'] ); ?>" title="<?php esc_attr_e( 'Delete', 'raybogman-content-orchestrator' ); ?>">
		...
		<div class="rbco-card">
			<div class="rbco-card-header">
				<h2>
					...
					<?php esc_html_e( 'Step 2: Content Creation', 'raybogman-content-orchestrator' ); ?>
				</h2>
			</div>
			<div class="rbco-card-body">
				<table class="form-table" role="presentation">
					<tbody>
						<tr>
							<th scope="row">
								<label for="rbco-prompt"><?php esc_html_e( 'Prompt', 'raybogman-content-orchestrator' ); ?></label>
							</th>
							<td>
								<textarea id="rbco-prompt" class="large-text" rows="5" placeholder="<?php esc_attr_e( "Describe what content to create...\ne.g. Write a blog post about the top 10 SEO strategies for small businesses", 'raybogman-content-orchestrator' ); ?>"></textarea>
								<p class="description">
									<?php esc_html_e( 'Be specific about the topic, tone, and target audience. The AI will use this prompt along with any scanned website data.', 'raybogman-content-orchestrator' ); ?>
								</p>
							</td>
						</tr>
						<tr>
							<th scope="row"><?php esc_html_e( 'Content Type', 'raybogman-content-orchestrator' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input type="radio" name="rbco-type" value="blog" checked="checked" />
										<strong><?php esc_html_e( 'Blog Post', 'raybogman-content-orchestrator' ); ?></strong>
										<span class="description">&mdash; <?php esc_html_e( 'targets 1000-2000 words, includes tags and categories', 'raybogman-content-orchestrator' ); ?></span>
									</label>
									<br>
									<label>
										<input type="radio" name="rbco-type" value="page" />
										<strong><?php esc_html_e( 'Page', 'raybogman-content-orchestrator' ); ?></strong>
										<span class="description">&mdash; <?php esc_html_e( 'targets 500-1000 words, no categories or tags', 'raybogman-content-orchestrator' ); ?></span>
									</label>
								</fieldset>
							</td>
						</tr>
						...
						<tr id="rbco-style-row">
							<th scope="row"><?php esc_html_e( 'Blog Style', 'raybogman-content-orchestrator' ); ?></th>
							<td>
								<div style="display: flex; align-items: center; gap: 8px; position: relative;">
									...
									<span id="rbco-style-preview-trigger" class="dashicons dashicons-visibility" title="<?php esc_attr_e( 'Hover to preview this style', 'raybogman-content-orchestrator' ); ?>" style="cursor: help; color: #2271b1; font-size: 20px; width: 20px; height: 20px;"></span>
									...
									<div id="rbco-style-preview-panel" class="rbco-style-preview-panel" style="display: none;">
										<div class="rbco-style-preview-header">
											...
											<a href="<?php echo esc_url( admin_url( 'admin.php?page=rbco-examples' ) ); ?>" target="_blank" style="font-size: 12px; margin-left: auto;">
												<?php esc_html_e( 'View all examples', 'raybogman-content-orchestrator' ); ?> &rarr;
											</a>
										</div>
						...
						<tr>
							<th scope="row"><?php esc_html_e( 'Publish Status', 'raybogman-content-orchestrator' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input type="radio" name="rbco-status" value="draft" checked="checked" />
										<strong><?php esc_html_e( 'Draft', 'raybogman-content-orchestrator' ); ?></strong>
										<span class="description">&mdash; <?php esc_html_e( 'save for review before publishing', 'raybogman-content-orchestrator' ); ?></span>
									</label>
									<br>
									<label>
										<input type="radio" name="rbco-status" value="publish" />
										<strong><?php esc_html_e( 'Publish', 'raybogman-content-orchestrator' ); ?></strong>
										<span class="description">&mdash; <?php esc_html_e( 'publish immediately (or at scheduled time)', 'raybogman-content-orchestrator' ); ?></span>
									</label>
								</fieldset>
							</td>
						</tr>
						...
						<tr>
							<th scope="row"><?php esc_html_e( 'Schedule', 'raybogman-content-orchestrator' ); ?></th>
							<td>
								<fieldset>
									<label>
										<input type="checkbox" id="rbco-schedule-enabled" />
										<strong><?php esc_html_e( 'Schedule for later', 'raybogman-content-orchestrator' ); ?></strong>
									</label>
									<div id="rbco-schedule-fields" style="display:none; margin-top: 10px;">
										...
										<p class="description">
											<?php esc_html_e( 'Site time zone:', 'raybogman-content-orchestrator' ); ?>
											<code><?php echo esc_html( wp_timezone_string() ); ?></code>
											&middot;
											<?php esc_html_e( 'Current time:', 'raybogman-content-orchestrator' ); ?>
											<code><?php echo esc_html( wp_date( 'Y-m-d H:i' ) ); ?></code>
										</p>
										<p class="description" id="rbco-schedule-help-draft">
											<span class="dashicons dashicons-info" style="color:#2271b1;"></span>
											<?php esc_html_e( 'Draft + Schedule = Review before publishing. The post will appear in your Scheduled list for you to approve first.', 'raybogman-content-orchestrator' ); ?>
										</p>
										<p class="description" id="rbco-schedule-help-publish" style="display:none;">
											<span class="dashicons dashicons-info" style="color:#2271b1;"></span>
											<?php esc_html_e( 'Publish + Schedule = Direct scheduled publication. WordPress will publish it automatically at the scheduled time.', 'raybogman-content-orchestrator' ); ?>
										</p>
									</div>
								</fieldset>
							</td>
								...
								<td>
									<fieldset>
										<label>
											...
											<strong><?php esc_html_e( 'Post to LinkedIn when published', 'raybogman-content-orchestrator' ); ?></strong>
										</label>
										<p class="description">
											<?php
											printf(
												/* translators: %s: LinkedIn account name */
												esc_html__( 'Will share to LinkedIn as %s when the content is published (immediately or after approval).', 'raybogman-content-orchestrator' ),
								...
								<td>
									<fieldset>
										<label>
											...
											<strong><?php esc_html_e( 'Post to Instagram when published', 'raybogman-content-orchestrator' ); ?></strong>
										</label>
										<p class="description">
											<?php
											printf(
            /* translators: %s: dynamic value */
												esc_html__( 'Will share to Instagram as @%s with the featured image and an AI-generated caption.', 'raybogman-content-orchestrator' ),
							...
							<td>
								<fieldset>
									<label>
										...
										<strong><?php esc_html_e( 'Generate AI featured image', 'raybogman-content-orchestrator' ); ?></strong>
									</label>
									<p class="description">
										<?php if ( $rbco_image_configured ) : ?>
											...
											printf(
												...
												esc_html__( 'Uses %s to generate a landscape image based on the blog topic. Set as the featured image — LinkedIn will automatically show this image when sharing.', 'raybogman-content-orchestrator' ),
										...
										<?php else : ?>
											...
											printf(
												...
												'<em>' . esc_html__( 'Requires an API key for %s. Configure it in Settings → Featured Image Provider section.', 'raybogman-content-orchestrator' ) . '</em>',
						...
						<tr>
							...
							<td>
								<fieldset>
									...
									<?php else : ?>
										<option value="thrive" disabled><?php esc_html_e( 'Thrive Architect (Enterprise)', 'raybogman-content-orchestrator' ); ?></option>
									<?php endif; ?>
									</select>
									<p class="description" id="rbco-output-format-desc">
										<?php esc_html_e( 'Choose how the content is formatted. WordPress (Standard) works with any theme. Thrive Architect creates content that\'s fully editable in Thrive\'s visual editor.', 'raybogman-content-orchestrator' ); ?>
									</p>
						...
						<tr id="rbco-categories-row">
							<th scope="row">
								<label for="rbco-categories"><?php esc_html_e( 'Categories', 'raybogman-content-orchestrator' ); ?></label>
							</th>
							<td>
								...
								<p class="description">
									<?php esc_html_e( 'Select categories to assign. The AI may also suggest additional categories which will be created automatically.', 'raybogman-content-orchestrator' ); ?>
								</p>
							</td>
						</tr>
					</tbody>
		...
		<div id="rbco-log-area" class="rbco-card" style="display: none;">
			<div class="rbco-card-header" style="display:flex; justify-content:space-between; align-items:center;">
				<h2 style="margin:0;">
					...
					<?php esc_html_e( 'Progress', 'raybogman-content-orchestrator' ); ?>
				</h2>
				<button type="button" class="button button-small rbco-save-log" data-log="#rbco-log-box" title="<?php esc_attr_e( 'Download log as text file', 'raybogman-content-orchestrator' ); ?>">
		...
		<div id="rbco-result-card" class="rbco-card rbco-result-card" style="display: none;">
			<div class="rbco-card-header rbco-card-header-success">
				<h2>
					...
					<?php esc_html_e( 'Content Created Successfully', 'raybogman-content-orchestrator' ); ?>
				</h2>
			</div>
			<div class="rbco-card-body">
				...
				<div id="rbco-result-actions" style="margin-top: 16px;">
					<a id="rbco-view-scheduled" href="<?php echo esc_url( admin_url( 'admin.php?page=rbco-scheduled' ) ); ?>" class="button button-primary" style="display:none;">
						...
						<?php esc_html_e( 'View Scheduled', 'raybogman-content-orchestrator' ); ?>
					</a>
					<a id="rbco-view-post" href="#" class="button button-primary" target="_blank">
						...
						<?php esc_html_e( 'View Post', 'raybogman-content-orchestrator' ); ?>
					</a>
					<a id="rbco-edit-post" href="#" class="button" target="_blank">
						...
						<?php esc_html_e( 'Edit in WordPress', 'raybogman-content-orchestrator' ); ?>
					</a>
					<button type="button" id="rbco-toggle-preview" class="button">
						...
						<?php esc_html_e( 'Toggle Preview', 'raybogman-content-orchestrator' ); ?>
					</button>
				</div>
