...
class AICC_Image_Overlay {
	...
	public static function create( $base_attachment_id, $title, $options = array() ) {
		if ( ! function_exists( 'imagecreatefromjpeg' ) ) {
			return new \WP_Error( 'no_gd', __( 'PHP GD extension is required for image overlay. Contact your hosting provider.', 'ai-content-orchestrator' ) );
		}
		...
		if ( ! $base_path || ! file_exists( $base_path ) ) {
			return new \WP_Error( 'no_base', __( 'Default featured image not found in media library.', 'ai-content-orchestrator' ) );
		}

		$info = getimagesize( $base_path );
		if ( ! $info ) {
			return new \WP_Error( 'invalid_image', __( 'Could not read the base image dimensions.', 'ai-content-orchestrator' ) );
		}
		...
		switch ( $mime ) {
			...
			case 'image/webp':
				if ( function_exists( 'imagecreatefromwebp' ) ) {
					...
					return new \WP_Error( 'unsupported', __( 'WebP support requires PHP 7.1+ with GD compiled with WebP.', 'ai-content-orchestrator' ) );
				}
				break;
			case 'image/avif':
				if ( function_exists( 'imagecreatefromavif' ) ) {
					...
					return new \WP_Error( 'unsupported', __( 'AVIF support requires PHP 8.1+ with GD compiled with AVIF. Please use a JPEG or PNG base image instead.', 'ai-content-orchestrator' ) );
				}
				break;
			default:
				return new \WP_Error( 'unsupported', __( 'Only JPEG, PNG, WebP, and AVIF base images are supported.', 'ai-content-orchestrator' ) );
		}

		if ( ! $img ) {
			return new \WP_Error( 'load_failed', __( 'Failed to load the base image with GD.', 'ai-content-orchestrator' ) );
		}
		...
		if ( empty( $font_bold ) || ! file_exists( $font_bold ) ) {
			imagedestroy( $img );
			return new \WP_Error( 'no_font', __( 'Bold font file (.ttf) not found. Upload a TTF font and configure it in Settings → Featured Image Provider.', 'ai-content-orchestrator' ) );
		}
		...
		$filename = 'aicc-featured-' . $slug . '-' . time() . '.jpg';
	...
	public static function create_and_attach( $post_id, $base_attachment_id, $title, $options = array() ) {
		...
		update_post_meta( $attach_id, '_aicc_generated_image', '1' );
		update_post_meta( $attach_id, '_aicc_overlay_image', '1' );
