...
class RBCO_Thrive_Converter {
	...
	public static function convert( $html ) {
		...
		$wrapped = '<?xml encoding="UTF-8"><div id="rbco-thrive-root">' . $html . '</div>';
		$dom->loadHTML( $wrapped, LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD );
		libxml_clear_errors();

		$root = $dom->getElementById( 'rbco-thrive-root' );
		if ( ! $root ) {
			return $html;
		}
	...
	public static function set_thrive_meta( $post_id, $thrive_content ) {
		...
		self::inject_user_template_css( $post_id, (int) RBCO_Settings::get_thrive_toc_id() );
		self::inject_user_template_css( $post_id, (int) RBCO_Settings::get_thrive_cta_symbol_id() );

		// Mark this post as AICC-generated in Thrive mode so we can identify it later.
		update_post_meta( $post_id, '_rbco_thrive_generated', '1' );
	}
